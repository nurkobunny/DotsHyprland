...
gsettings set org.gnome.desktop.interface gtk-theme "Rosepine-BL-LB-Dark-Moon"
...
sed -i 's/^.*col\.active_border.*/col.active_border = rgb(c4a7e7)/' "$USER_DECOR"
sed -i 's/^.*col\.inactive_border.*/col.inactive_border = rgb(232136)/' "$USER_DECOR"

WALL_DIR="$HOME/Pictures/wallpapers/Dynamic-Wallpapers/RosePine/"
...
HYPRLOCK_CONFIG="$HOME/.config/hypr/hyprlockthemes/RosePineMoon.conf"
[ -f "$HYPRLOCK_CONFIG" ] && cp "$HYPRLOCK_CONFIG" "$HOME/.config/hypr/hyprlock.conf"

sudo sed -i 's/ConfigFile=Themes\/.*/ConfigFile=Themes\/RosePineMoon.conf/' /usr/share/sddm/themes/simple_sddm_2/metadata.desktop
sudo cp "$HOME"/Pictures/wallpapers/current/* /usr/share/sddm/themes/simple_sddm_2/Backgrounds/default

WALLALLDIREC="$HOME/Pictures/fastfetch/rosepine"
...
kitten themes --reload-in=all Rosé Pine Moon
...
if [ -f "$NVIM_CONFIG" ]; then
    ...
    sed -i "s/variant = '.*'/variant = 'moon'/" "$NVIM_CONFIG"
    sed -i "s/^colorscheme .*$/colorscheme rose-pine-moon/" "$NVIM_CONFIG"
    pkill -x nvim
fi
...
if [ -f "$VSCODE_SETTINGS" ]; then
    sed -i 's/"workbench.colorTheme": *".*"/"workbench.colorTheme": "Rosé Pine Moon"/' "$VSCODE_SETTINGS"
fi

ROFI_CONFIG="$HOME/.config/rofi/style.rasi"
sed -i 's|^@theme.*|@theme "~/.config/rofi/wallust/RosePineMoon.rasi"|' "$ROFI_CONFIG"

PROFILE_DIR=$(find "$HOME/.mozilla/firefox" -maxdepth 1 -type d -name "*.default-release" | head -n 1)
if [ -d "$PROFILE_DIR" ]; then
    cp -f "$HOME/.config/firefox-themes/RosePineMoon/userChrome.css" "$PROFILE_DIR/chrome/"
    cp -f "$HOME/.config/firefox-themes/RosePineMoon/index.html" "$HOME/.config/firefox-custom-addons/index.html"
    pkill -x firefox && nohup firefox &> /dev/null &
fi

pkill swaync
cp "$HOME/.config/swaync/Themes/RosePineMoon.css" "$HOME/.config/swaync/style.css"
swaync & disown

spicetify config color_scheme rosepine-moon
spicetify apply -n
...
cp "$HOME/.config/waybar/style/RosePineMoon.css" "$HOME/.config/waybar/style.css"
