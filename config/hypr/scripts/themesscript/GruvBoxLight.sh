...
COLOR_HEX="#C26318"
COLOR_ANSI="3"
cp $HOME/.config/gtk-3.0/Themes/GruvBoxLight.css $HOME/.config/gtk-3.0/gtk.css* 
gsettings set org.gnome.desktop.interface cursor-theme 'phinger-cursors-gruvbox-material-light'
gsettings set org.gnome.desktop.interface cursor-size 24
hyprctl setcursor phinger-cursors-gruvbox-material-light 24
sed -i "s/^exec-once=hyprctl setcursor .*/exec-once=hyprctl setcursor phinger-cursors-gruvbox-material-light 24/" "$HOME/.config/hypr/UserConfigs/Startup_Apps.conf"
mv $HOME/.config/gtk-3.0/gtk.css* $HOME/.config/gtk-3.0/gtk.css
gsettings set org.gnome.desktop.interface gtk-theme "Gruvbox-BL-LB-Light-Soft"
...
sed -i 's/^.*col\.active_border.*/col.active_border = rgb(b57614)/' "$USER_DECOR"
sed -i 's/^.*col\.inactive_border.*/col.inactive_border = rgb(fbf1c7)/' "$USER_DECOR"

WALL_DIR="$HOME/Pictures/wallpapers/Dynamic-Wallpapers/GruvBoxLight/"
...
HYPRLOCK_CONFIG="$HOME/.config/hypr/hyprlockthemes/GruvBoxLight.conf"
[ -f "$HYPRLOCK_CONFIG" ] && cp "$HYPRLOCK_CONFIG" "$HOME/.config/hypr/hyprlock.conf"

sudo sed -i 's/ConfigFile=Themes\/.*/ConfigFile=Themes\/GruvBoxMaterialLight.conf/' /usr/share/sddm/themes/simple_sddm_2/metadata.desktop
...
kitten themes --reload-in=all Gruvbox Material Light Soft
...
if [ -f "$NVIM_CONFIG" ]; then
    ...
    sed -i 's/set background=.*/set background=light/' "$NVIM_CONFIG"
    pkill -x nvim
fi
...
if [ -f "$VSCODE_SETTINGS" ]; then
    sed -i 's/"workbench.colorTheme": *".*"/"workbench.colorTheme": "Gruvbox Light Soft"/' "$VSCODE_SETTINGS"
fi
...
if [ -d "$PROFILE_DIR" ]; then
    cp -f "$HOME/.config/firefox-themes/gruvboxlight/userChrome.css" "$PROFILE_DIR/chrome/"
    cp -f "$HOME/.config/firefox-themes/gruvboxlight/index.html" "$HOME/.config/firefox-custom-addons/index.html"
    pkill -x firefox && nohup firefox &> /dev/null &
fi

pkill swaync
cp "$HOME/.config/swaync/Themes/GruvBoxLight.css" "$HOME/.config/swaync/style.css"
swaync & disown

spicetify config color_scheme gruvbox-material-light-soft
spicetify apply
...
cp "$HOME/.config/waybar/style/GruvBoxLight.css" "$HOME/.config/waybar/style.css"
