...
sed -i "s/^exec-once=hyprctl setcursor .*/exec-once=hyprctl setcursor Nordic-cursors 24/" "$HOME/.config/hypr/UserConfigs/Startup_Apps.conf"
...
sed -i 's/^.*col\.active_border.*/col.active_border = rgb(81a1c1)/' "$USER_DECOR"
sed -i 's/^.*col\.inactive_border.*/col.inactive_border = rgb(2e3440)/' "$USER_DECOR"

WALL_DIR="$HOME/Pictures/wallpapers/Dynamic-Wallpapers/Nord/"
...
HYPRLOCK_CONFIG="$HOME/.config/hypr/hyprlockthemes/Nord.conf"
[ -f "$HYPRLOCK_CONFIG" ] && cp "$HYPRLOCK_CONFIG" "$HOME/.config/hypr/hyprlock.conf"

sudo sed -i 's/ConfigFile=Themes\/.*/ConfigFile=Themes\/Nordic.conf/' /usr/share/sddm/themes/simple_sddm_2/metadata.desktop
sudo cp "$HOME"/Pictures/wallpapers/current/* /usr/share/sddm/themes/simple_sddm_2/Backgrounds/default

WALLALLDIREC="$HOME/Pictures/fastfetch/nord"
...
kitten themes --reload-in=all Nord
...
if [ -f "$NVIM_CONFIG" ]; then
    sed -i "s/theme = '.*'/theme = 'nord'/" "$NVIM_CONFIG"
    sed -i "s/^colorscheme .*$/colorscheme nord/" "$NVIM_CONFIG"
    pkill -x nvim
fi

OBSIDIAN_CONFIG="$OBSIDIAN_VAULT.obsidian/appearance.json"
if [ -f "$OBSIDIAN_CONFIG" ]; then
    sed -i 's/"cssTheme": *".*"/"cssTheme": "Nord"/' "$OBSIDIAN_CONFIG"
...
if [ -f "$VSCODE_SETTINGS" ]; then
    sed -i 's/"workbench.colorTheme": *".*"/"workbench.colorTheme": "Nord"/' "$VSCODE_SETTINGS"
fi

ROFI_CONFIG="$HOME/.config/rofi/style.rasi"
sed -i 's|^@theme.*|@theme "~/.config/rofi/wallust/Nord.rasi"|' "$ROFI_CONFIG"

PROFILE_DIR=$(find "$HOME/.mozilla/firefox" -maxdepth 1 -type d -name "*.default-release" | head -n 1)
if [ -d "$PROFILE_DIR" ]; then
    cp -f "$HOME/.config/firefox-themes/nord/userChrome.css" "$PROFILE_DIR/chrome/"
    cp -f "$HOME/.config/firefox-themes/nord/index.html" "$HOME/.config/firefox-custom-addons/index.html"
    pkill -x firefox && nohup firefox &> /dev/null &
fi

pkill swaync
cp "$HOME/.config/swaync/Themes/Nord.css" "$HOME/.config/swaync/style.css"
swaync & disown
...
cp "$HOME/.config/waybar/style/Nord.css" "$HOME/.config/waybar/style.css"
